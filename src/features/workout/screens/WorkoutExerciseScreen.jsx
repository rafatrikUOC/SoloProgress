...
export default function WorkoutExerciseScreen({ navigation, route }) {
    ...
    // Flag to control if sets can be marked (based on session start_time)
    const [canMarkSets, setCanMarkSets] = useState(false);

    // Check if the session has NOT started (start_time is null)
    useEffect(() => {
        if (!trainingSessionId) return;
        let mounted = true;
        async function checkSessionStarted() {
            const { data, error } = await supabase
                ...
                .single();
            if (mounted) {
                // Only allow marking sets if start_time is null (session not started)
                setCanMarkSets(data && data.start_time);
            }
        }
        checkSessionStarted();
    ...
    const handleDeleteSeries = async (seriesKey) => {
        const realSeries = series.find(s => getSeriesUniqueKey(s) === seriesKey);
        if (realSeries) {
            await supabase.from("ExerciseSeries")
                .delete()
                .eq("training_exercise_id", realSeries.training_exercise_id)
                .eq('"order"', realSeries.order)
                .eq("is_warmup", realSeries.is_warmup);
    ...
    const renderSetRow = (s, idx, isWarmup) => {
        ...
        return (
            ...
            >
                <View style={[
                    styles.pillRow,
                    isCompleted && canMarkSets && styles.pillRowCompleted
                ]}>
                    {/* SET NUMBER */}
                    <View style={styles.pillColSet}>
                        <TouchableOpacity
                            style={[
                                styles.pill,
                                isCompleted && canMarkSets && styles.pillCompleted
                            ]}
                            onPress={() => canMarkSets && toggleSetCompleted(uniqueKey)}
                            activeOpacity={canMarkSets ? 0.7 : 1}
                        >
                            <Text
                                style={[
                                    styles.pillText,
                                    isCompleted && canMarkSets && styles.pillTextCompleted
                                ]}>
                                {idx + 1}
                            </Text>
                        </TouchableOpacity>
                    ...
                    <View style={styles.pillColInput}>
                        <TextInput
                            style={[
                                styles.pillInput,
                                isCompleted && canMarkSets && styles.pillCompletedInput
                            ]}
